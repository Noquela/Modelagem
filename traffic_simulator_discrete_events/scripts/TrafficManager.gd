...
func update_cycle_times(new_times: Dictionary):
	"""Atualiza os tempos dos ciclos dos semáforos"""
	print("🚦 TrafficManager atualizando ciclos de semáforos...")
	
	# Atualizar as constantes
	if new_times.has("main_road_green"):
		CYCLE_TIMES.main_road_green = new_times.main_road_green
	if new_times.has("cross_road_green"):
		CYCLE_TIMES.cross_road_green = new_times.cross_road_green
	if new_times.has("yellow_time"):
		CYCLE_TIMES.yellow_time = new_times.yellow_time
	if new_times.has("safety_time"):
		CYCLE_TIMES.safety_time = new_times.safety_time
	if new_times.has("total_cycle"):
		CYCLE_TIMES.total_cycle = new_times.total_cycle
	
	print("✅ Novos tempos aplicados:")
	print("   - Principal Verde: %.0fs" % CYCLE_TIMES.main_road_green)
	print("   - Transversal Verde: %.0fs" % CYCLE_TIMES.cross_road_green)
	print("   - Amarelo: %.1fs" % CYCLE_TIMES.yellow_time)
	print("   - Segurança: %.1fs" % CYCLE_TIMES.safety_time)
	print("   - Ciclo Total: %.1fs" % CYCLE_TIMES.total_cycle)
	
	# Reiniciar ciclo para aplicar mudanças imediatamente
	cycle_start_time = Time.get_unix_time_from_system()
