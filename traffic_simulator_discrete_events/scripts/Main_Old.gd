...
# INTEGRAÇÃO COM EVENTOS DISCRETOS LIMPOS
var traffic_simulator: TrafficSimulator
var is_running: bool = true

func _ready():
	print("🌍 Main iniciando com mundo visual + nova arquitetura de eventos discretos")
	
	# Criar TrafficSimulator novo e limpo
	traffic_simulator = TrafficSimulator.new()
	add_child(traffic_simulator)
	...
	print("✅ Mundo visual + eventos discretos integrados!")
...
func _on_stats_updated(stats: Dictionary):
	# EVENTOS DISCRETOS: Analytics é atualizado via sinais do gerenciador de eventos
	# Não precisa mais da função update_display
	pass

func toggle_pause():
	if traffic_simulator:
		if is_running:
			traffic_simulator.pause_simulation()
		else:
			traffic_simulator.resume_simulation()
