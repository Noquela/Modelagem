...
var car_id: int = 0  # IMPORTANTE: Inicializar para evitar erro de assignment
...
# HÍBRIDO: Controle discreto vs contínuo
...
func create_car_geometry():
	...
	# Collision shape (ajustado para modo híbrido)
	var collision = CollisionShape3D.new()
	var box_shape = BoxShape3D.new()
	
	if hybrid_mode:
		# Collision maior para carros híbridos (mas menor que o visual)
		box_shape.size = Vector3(1.5, 1.2, 3.5)  # Ligeiramente menor que o visual
	else:
		box_shape.size = Vector3(0.8, 0.4, 1.5)  # Tamanho original para contínuo
	
	collision.shape = box_shape
	add_child(collision)

func create_fallback_car_mesh():
	# CARROS HÍBRIDOS: Muito maiores e mais visíveis!
	var car_body = MeshInstance3D.new()
	var body_mesh = BoxMesh.new()
	
	# TAMANHO AINDA MAIOR para híbrido
	if hybrid_mode:
		body_mesh.size = Vector3(2.2, 1.8, 4.8)  # ENORME para máxima visibilidade
		print("🚗 Car %d: Creating EXTRA LARGE hybrid fallback mesh: %s" % [car_id, body_mesh.size])
	else:
		body_mesh.size = Vector3(1.0, 0.6, 2.0)  # Maior que antes para contínuo
	...
	var car_colors = [
		...
		Color.SILVER, Color.GRAY, Color.GREEN, Color.YELLOW
	]
	...
	# Add a front indicator (direção do movimento)
	var front_indicator = MeshInstance3D.new()
	var indicator_mesh = BoxMesh.new()
	
	if hybrid_mode:
		indicator_mesh.size = Vector3(2.2, 1.2, 0.5)  # Indicador proporcional
		front_indicator.position = Vector3(0, 0.8, -2.6)  # Frente do carro enorme
	else:
		indicator_mesh.size = Vector3(1.0, 0.4, 0.3)  # Maior para contínuo
		front_indicator.position = Vector3(0, 0.3, -1.2)
...
func create_3d_car_model() -> void:
	# TESTAR PRIMEIRO se os assets existem
	test_car_assets()
	
	# USAR TODOS OS MODELOS 3D agora que estão funcionando
	var car_models = [
	...
	print("🚗 Car %d: Loading 3D model: %s" % [car_id, model_path])
	...
	if car_scene and car_scene.can_instantiate():
		...
		# TAMANHO REALISTA para carros bem visíveis
		if hybrid_mode:
			car_model.scale = Vector3(1.2, 1.2, 1.2)  # AINDA MAIOR - carros realistas
			car_model.position = Vector3(0, 0, 0)     # Centralizado
			print("🚗 Car %d: HYBRID 3D model %s - scale %s" % [car_id, model_path, car_model.scale])
		else:
			car_model.scale = Vector3(0.6, 0.6, 0.6)  # Maior que antes para contínuo
			car_model.position = Vector3(0, -0.2, 0)
		...
		# DEBUG: Mostrar qual modelo falhou
		print("❌ Car %d: Failed to load 3D model: %s" % [car_id, model_path])
		print("🔄 Car %d: Using fallback mesh instead" % car_id)
		create_fallback_car_mesh()

func test_car_assets():
	"""Testa se os assets GLB podem ser carregados"""
	var test_models = [
		"res://assets/vehicles/sedan.glb",
		"res://assets/vehicles/hatchback-sports.glb", 
		"res://assets/vehicles/suv.glb",
	...
	print("🔍 Testing car assets for Car %d:" % car_id)
	for model_path in test_models:
		var scene = load(model_path)
		if scene:
			print("  ✅ %s - OK" % model_path)
		else:
			print("  ❌ %s - FAILED" % model_path)
...
func _physics_process(delta):
	# HÍBRIDO: Só executar lógica contínua se NÃO estiver em modo híbrido
	if hybrid_mode:
		return
...
func get_car_ahead():
	...
	for car in cars:
		...
		# DEBUG: Para carros NS, mostrar quando encontra outro carro na mesma lane
		if get_direction_name() == "South" and car_id % 200 == 0 and fmod(Engine.get_process_frames(), 300) == 0:
			print("DEBUG Car #%d (South): found other car #%d in same lane, pos=%s" % [car_id, other_car.car_id, other_car.global_position])
		...
		match direction:
			...
			Direction.BOTTOM_TO_TOP:
				# South→North, carro à frente tem Z MENOR (mais próximo da interseção)
				is_ahead = other_car.global_position.z < global_position.z
		...
		if is_ahead and distance < closest_distance:
			...
			# DEBUG: Para carros NS, mostrar quando identifica carro à frente
			if get_direction_name() == "South" and car_id % 200 == 0 and fmod(Engine.get_process_frames(), 300) == 0:
				print("DEBUG Car #%d (South): car #%d is AHEAD, distance=%.1f" % [car_id, other_car.car_id, distance])
...
func collision_avoidance_behavior() -> Dictionary:
	...
	if not car_ahead:
		# DEBUG: Para carros NS, mostrar quando não há carro à frente
		if get_direction_name() == "South" and car_id % 100 == 0 and fmod(Engine.get_process_frames(), 180) == 0:
			print("DEBUG Car #%d (South): NO car ahead detected" % car_id)
		return result
	...
	# DEBUG: Para carros NS, mostrar detecção de carros à frente
	if get_direction_name() == "South" and car_id % 100 == 0 and fmod(Engine.get_process_frames(), 180) == 0:
		print("DEBUG Car #%d (South): car_ahead detected, distance=%.1f, safe=%.1f" % [car_id, distance, safe_distance])
...
func traffic_light_behavior() -> Dictionary:
	...
	# DEBUG: Para carros Sul→Norte, mostrar estado do semáforo
	if get_direction_name() == "South" and car_id % 50 == 0 and fmod(Engine.get_process_frames(), 120) == 0:
		var front_pos = get_car_front_position()
		print("DEBUG Car #%d (South): light_state=%s, stop_distance=%.1f, pos=%s, front_pos=%s" % [car_id, light_state, stop_distance_front, global_position, front_pos])
	...
	match light_state:
		"red":
			# SEMPRE para no vermelho se ainda não passou da linha
			if stop_distance_front > 0:
				result.should_stop = true
		"yellow":
...
func hasPassedIntersection() -> bool:
	...
	match direction:
		...
		Direction.BOTTOM_TO_TOP:
			return global_position.z < -4.0    # South→North: passou quando Z < -4
	return false
...
func get_car_front_position() -> Vector3:
	...
	match direction:
		...
		Direction.BOTTOM_TO_TOP:
			front_offset = Vector3(0, 0, -car_length/2)  # Sul→Norte: frente aponta para -Z
...
func get_distance_front_to_stop_line() -> float:
	...
	match direction:
		...
		Direction.BOTTOM_TO_TOP:  # Moving South to North
			# Crosswalk at z=-7.0, want to stop at z=-9.0 (2m before) - COPIANDO LEFT_TO_RIGHT
			var target_stop_z = -7.0 - safety_buffer  # -9.0
			return max(0.0, target_stop_z - front_pos.z)  # Distance to reach -9.0 - IGUAL LEFT_TO_RIGHT
...
func update_movement(delta):
	...
	# DEBUG: Para carros NS, mostrar aplicação do IDM
	if get_direction_name() == "South" and car_id % 150 == 0 and fmod(Engine.get_process_frames(), 240) == 0:
		var has_leader = leader_car != null
		print("DEBUG Car #%d (South) IDM: leader=%s, accel=%.2f, current_speed=%.2f" % [car_id, has_leader, acceleration, current_speed])
...
func set_hybrid_mode(enabled: bool):
	"""Ativa/desativa controle híbrido"""
	hybrid_mode = enabled
	discrete_controlled = enabled
	
	if hybrid_mode:
		print("🎯 Car %d: HYBRID MODE enabled" % car_id)
		# Garantir que geometria existe
		if not has_method("create_car_geometry"):
			print("⚠️ Car %d: create_car_geometry method not found!" % car_id)
		# Desabilitar IA contínua mantendo apenas atualizações visuais
		# _physics_process já vai retornar early
	else:
		print("🔄 Car %d: CONTINUOUS MODE enabled" % car_id)

func set_visual_speed(speed: float):
	"""Define velocidade visual para compatibilidade"""
	visual_speed = speed
	
	# Atualizar current_speed para analytics
	if hybrid_mode:
		current_speed = visual_speed

func get_current_speed() -> float:
	"""Compatibilidade com analytics (funciona em ambos modos)"""
